...
static int rvpmu_deleg_find_ctrs(void)
{
	...
	if (!(cmask & RISCV_PMU_FIXED_COUNTER_MASK))
		pr_err("The implementation doesn't have cycle/instret counter which is mandatory");
...
static int get_deleg_fixed_hw_idx(unsigned long event_value)
{
	...
	if (event_value == PERF_COUNT_HW_CPU_CYCLES)
		cidx = 0;
	else if (event_value == PERF_COUNT_HW_INSTRUCTIONS)
		cidx = 2;

	/* Do a sanity check if the hardware is compliant to priv spec or not */
	if ((cidx == 0 || cidx == 2) && (cmask & (1 << cidx)))
		return cidx;
	else
		return -EINVAL;
}
